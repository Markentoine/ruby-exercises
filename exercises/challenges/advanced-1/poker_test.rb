...
class PokerTest < Minitest::Test
  ...
  def test_two_pair_w_same_kicker
    pair_of_2 = %w(4H 4D 3S 9S JH)
    pair_of_4 = %w(4H 4D 2S 9S JH)
    game = Poker.new([pair_of_2, pair_of_4])
    assert_equal [pair_of_2], game.best_hand
  end
  ...
  def test_two_full_houses
    full_hi = %w(4S 5H 4S 5D 4H)
    full_low = %w(3S 3H 8S 8D 3H)
    game = Poker.new([full_hi, full_low])
    assert_equal [full_hi], game.best_hand
  end
  ...
  def test_one_pair_vs_one_pair_equals_different_by_last_kicker
    kicker_5 = %w(4S 5H 4C 8D KH)
    kicker_3 = %w(8S 4H 3S 4D KH)
    game = Poker.new([kicker_5, kicker_3])
    assert_equal [kicker_5], game.best_hand
  end

  def test_two_pair_of_4_and6_different_kicker
    pair_of_4_6_J_kicker = %w(6D 4H 6S 4D JC)
    pair_of_4_6_K_Kicker = %w(6H 4S 6C 4C KH)
    game = Poker.new([pair_of_4_6_J_kicker, pair_of_4_6_K_Kicker])
    assert_equal [pair_of_4_6_K_Kicker], game.best_hand
  end

  def test_two_double_pair_same_value_different_kicker
    double_pair_4_and_8_J_kicker = %w(4D 8H 4H 8D JH)
    double_pair_4_and_8_5_kicker = %w(4S 8S 4C 8C 5H)
    game = Poker.new([double_pair_4_and_8_J_kicker, double_pair_4_and_8_5_kicker])
    assert_equal [double_pair_4_and_8_J_kicker], game.best_hand
  end

  def test_two_fulls_same_three_of_a_kind
    full_of_5_by_9 = %w(5C 5S 5H 9S 9D)
    full_of_5_by_8 = %w(5H 5S 5D 8S 8D)
    game = Poker.new([full_of_5_by_9, full_of_5_by_8])
    assert_equal [full_of_5_by_9], game.best_hand
  end
end
